...
| `LifeTime`<br/> [optional]        | The lifetime of the app context in milliseconds.                                                                              | Browser History defaults to `-1` (non-expiring), preserves `-1` and explicit positive values, and rejects other non-positive values. Other app context types default to 5 minutes, are capped at 5 minutes, and reject explicit non-positive values through `onContextResponseError`. |
